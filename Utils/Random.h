...
inline void shuffle(int *arr, const int n; RNG & rng){
    for(int i=n-1; i>0; i--){
        //A random non-negative integer <=1
...
int create_shuffled_batches(const Eigen::MatrixBase<DerivedX> & x,
    ...
    if(y.cols() != nobs){
        throw std::invalid_argument("Input X and Y have different numbers of observations");
    }
