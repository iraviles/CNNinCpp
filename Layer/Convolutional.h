...
private:
    ...
    Vector m_df_data;       //Derivada del filtro

    Vector m_bias;
    Vector m_db;            //Derivada de bias
...
public:
    ...
    void forward(const Matrix & prev_layer_data){
            ...
            //Se usarán valid Convoluions
            internal::convolve_valid(m_dim, prev_layer_data.data(), true, nobs, m_filter_data.data(), m_z.data());
            //No es necesario añadir bias porque ya se hizo en el operador de convolución

            int channel_start_row = 0;
            const int channel_nelem = m_dim.conv_rows * m_dim.channel_cols;

            for (int i=0; i<m_dim.out_channels, i++, channel_starr_row += channel_nelem){
